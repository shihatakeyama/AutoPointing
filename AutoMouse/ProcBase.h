...
public:
	...
	ProcBase(enum E_ProcType ProcType);
	virtual ~ProcBase() = 0;
	...
	// 処理オブジェクトを生成
	static ProcBase *newProc(enum E_ProcType ValTypeId);
	...
	// XMLからプロセスリストを読み込み
	static int32_t loadProcList(std::vector<ProcBase*>	&List ,rapidxml::node_t* ProcessXml/*-- ,sheet_t *Db*/);
	static int32_t saveProcList(const std::vector<ProcBase*> &List ,rapidxml::document_t &Doc ,rapidxml::node_t* ProcessXml);
